...
export function ImageToPdfTool({ tool }: { tool: ToolDefinition }) {
  ...
  const handleFilesSelected = (files: FileList | null) => {
    ...
    if (newItems.length === 0) {
      setErrorMessage(language === 'bn' ? 'অনুগ্রহ করে JPG, PNG বা WebP ছবি সিলেক্ট করুন।' : 'Please select valid JPG, PNG, or WebP images.');
      setStatus('error');
      return;
    }
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      for (const item of images) {
        // Universal image loading via canvas to support PNG, WebP, and JPG uniformly
        const img = new Image();
      ...
      trackEvent('tool_run', { tool: 'image-to-pdf', count: images.length });
    } catch (err) {
      console.error(err);
      setStatus('error');
      setErrorMessage(language === 'bn' ? 'PDF তৈরি করতে সমস্যা হয়েছে।' : 'Failed to generate PDF from images.');
    }
  };
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = 'ahadex-document.pdf';
    ...
    <main className="prose-page" style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 16px' }}>
      <header style={{ marginBottom: 28 }}>
        <span className="eyebrow" style={{ color: 'hsl(var(--primary))' }}>
          {tool.category} / {isBn ? 'মাল্টি-ফরম্যাট ছবি থেকে PDF' : 'Multi-format Image to PDF'}
        </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          >
            ...
            <h3 style={{ fontSize: 17, fontWeight: 700, margin: '0 0 6px' }}>
              {images.length > 0
                ? isBn ? 'আরও ছবি যোগ করুন (JPG, PNG, WebP)' : 'Add More Images (JPG, PNG, WebP)'
                : isBn ? 'আপনার ছবিগুলো নির্বাচন বা ড্র্যাগ করুন' : 'Select or Drag & Drop Images'}
            </h3>
            <span style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))' }}>
              {isBn ? 'যেকোনো মাপ ও ফরম্যাটের একাধিক ছবি একসাথে নির্বাচন করা যাবে' : 'Supports multiple mixed-format images'}
            </span>
          </div>
            ...
            >
              <div>
                ...
                >
                  ...
                  <option value="fit">{isBn ? 'ছবির নিজস্ব মাপে (Fit Image)' : 'Fit to Original Dimensions'}</option>
                </select>
              </div>
          ...
          {images.length > 0 && status !== 'done' && (
            ...
            >
              ...
              {status === 'generating'
                ...
                : isBn ? 'PDF তৈরি করুন' : 'Convert All to PDF'}
            </button>
          )}
          ...
          {status === 'done' && pdfBlob && (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))', display: 'block' }}>
                    {formatBytes(pdfBlob.size)} • {images.length} {isBn ? 'টি ছবি অন্তর্ভুক্ত' : 'images combined'}
                  </span>
                </div>
              </div>
              <button
              ...
              >
                <Download size={16} /> {isBn ? 'PDF ডাউনলোড করুন' : 'Download Combined PDF'}
              </button>
            </div>
          )}
      ...
      <section className="tool-content" style={{ marginTop: 40, borderTop: '1px solid hsl(var(--border))', paddingTop: 28 }}>
        <h2>{isBn ? 'কীভাবে ছবিকে PDF ডকুমেন্টে রূপান্তর করবেন' : 'How to Convert Images to PDF'}</h2>
        <ol>

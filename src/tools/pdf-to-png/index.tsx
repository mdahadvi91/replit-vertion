...
export function PdfToPngTool({ tool }: { tool: ToolDefinition }) {
  ...
  const [scale, setScale] = useState<number>(2.0); // 2x crisp retina
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      trackEvent('tool_run', { tool: 'pdf-to-png', pages: numPages });
    } catch (err) {
      console.error(err);
      setStatus('error');
      setErrorMessage(language === 'bn' ? 'PNG তৈরিতে সমস্যা হয়েছে।' : 'Error converting PDF pages to PNG.');
    }
  };
  ...
  const handleDownloadSingle = (page: RenderedPngPage) => {
    ...
    link.download = `${baseName}-page-${page.pageNumber}.png`;
  ...
  const handleDownloadZip = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `${baseName}-png-images.zip`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    <main className="prose-page" style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 16px' }}>
      <header style={{ marginBottom: 28 }}>
        <span className="eyebrow" style={{ color: 'hsl(var(--primary))' }}>
          {tool.category} / {isBn ? 'হাই-রেজোলিউশন ক্রিস্প PNG' : 'Lossless High-DPI PNG'}
        </span>
        ...
        {!file ? (
          ...
          >
            ...
            <h3 style={{ fontSize: 18, fontWeight: 700, margin: '0 0 8px' }}>
              {isBn ? 'আপনার PDF ফাইল নির্বাচন বা ড্র্যাগ করুন' : 'Select PDF File to Convert to PNG'}
            </h3>
            <p style={{ fontSize: 14, color: 'hsl(var(--muted-foreground))', margin: '0 0 18px' }}>
              {isBn ? 'প্রতিটি পৃষ্ঠা ক্রিস্প ও স্পষ্ট PNG ছবি হিসেবে ডাউনলোড করুন' : 'Export each page as a lossless, high-DPI transparent PNG image'}
            </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            {status !== 'done' && (
              <div style={{ maxWidth: 300 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>
                  {isBn ? 'PNG রেজোলিউশন / কোয়ালিটি' : 'PNG Resolution Quality'}
                </label>
                <select
                  value={scale}
                ...
                >
                  <option value={1.5}>{isBn ? 'স্ট্যান্ডার্ড (1.5x HD)' : 'Standard HD (150 DPI)'}</option>
                  <option value={2.0}>{isBn ? 'রেটিনা (2.0x Ultra HD - প্রস্তাবিত)' : 'Retina 2x (Recommended)'}</option>
                  <option value={3.0}>{isBn ? 'সর্বোচ্চ ক্রিস্প (3.0x Max Print)' : 'Max Quality (300 DPI)'}</option>
                </select>
              </div>
            )}
            ...
            {status === 'converting' && (
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                ...
                <span style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))' }}>
                  {isBn ? `পৃষ্ঠাগুলো PNG তে রেন্ডার হচ্ছে... (${progress}%)` : `Rendering pages to crisp PNG... (${progress}%)`}
                </span>
              </div>
            )}
            ...
            {status !== 'converting' && status !== 'done' && (
              ...
              >
                <Sparkles size={16} /> {isBn ? 'PNG ছবিতে রূপান্তর করুন' : 'Convert All Pages to PNG'}
              </button>
            )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <strong style={{ fontSize: 16 }}>{isBn ? 'রূপান্তর সফল হয়েছে!' : 'PNG Export Complete!'}</strong>
                      <span style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))', display: 'block' }}>
                        {pages.length} {isBn ? 'টি পৃষ্ঠা উচ্চ রেজোলিউশনের PNG তে রেন্ডার হয়েছে' : 'pages rendered to crisp lossless PNG'}
                      </span>
                    </div>
                  </div>
                  {zipBlob && (
                  ...
                  {pages.map((p) => (
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
                        ...
                        >
                          <Download size={13} /> {isBn ? 'PNG' : 'Download'}
                        </button>
                      </div>
                    </div>
                  ))}
      ...
      <section className="tool-content" style={{ marginTop: 40, borderTop: '1px solid hsl(var(--border))', paddingTop: 28 }}>
        <h2>{isBn ? 'কীভাবে PDF থেকে PNG ছবিতে রূপান্তর করবেন' : 'How to Convert PDF to PNG Images'}</h2>
        <ol>

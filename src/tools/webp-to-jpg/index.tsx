...
export function WebpToJpgTool({ tool }: { tool: ToolDefinition }) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected.type !== 'image/webp' && !selected.name.toLowerCase().endsWith('.webp')) {
      setErrorMessage(language === 'bn' ? 'অনুগ্রহ করে একটি WebP ফরম্যাটের ছবি নির্বাচন করুন।' : 'Please select a WebP image.');
      setStatus('error');
      return;
    }
  ...
  const handleConvert = () => {
    ...
    img.onload = () => {
      ...
      if (!ctx) {
        setStatus('error');
        setErrorMessage('Failed to access canvas context.');
      ...
      canvas.toBlob(
        (blob) => {
          ...
          trackEvent('tool_run', { tool: 'webp-to-jpg' });
        },
        'image/jpeg',
        quality
      );
    ...
    img.onerror = () => {
      URL.revokeObjectURL(objectUrl);
      setStatus('error');
      setErrorMessage(language === 'bn' ? 'ছবিটি লোড করা সম্ভব হয়নি।' : 'Failed to load WebP image.');
    };
    ...
    <main className="prose-page" style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 16px' }}>
      <header style={{ marginBottom: 28 }}>
        <span className="eyebrow" style={{ color: 'hsl(var(--primary))' }}>
          {tool.category} / {isBn ? 'সবার জন্য উন্মুক্ত ও দ্রুত' : 'Universal JPG Output'}
        </span>
        ...
        {!file ? (
          ...
          >
            ...
            <h3 style={{ fontSize: 18, fontWeight: 700, margin: '0 0 8px' }}>
              {isBn ? 'আপনার WebP ছবি নির্বাচন বা ড্র্যাগ করুন' : 'Select WebP Image to Convert to JPG'}
            </h3>
            <p style={{ fontSize: 14, color: 'hsl(var(--muted-foreground))', margin: '0 0 18px' }}>
              {isBn ? 'ওয়েব থেকে সেভ করা WebP ছবি এক ক্লিকে সাধারণ JPG তে রূপান্তর করুন' : 'Easily convert downloaded web images into universally compatible JPEG'}
            </p>
            <button type="button" className="button button-primary">
              <Upload size={16} /> {isBn ? 'WebP ছবি নির্বাচন করুন' : 'Choose WebP File'}
            </button>
          </div>
        ) : (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))' }}>
                    {formatBytes(file.size)} • Google WebP Image
                  </span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <strong style={{ fontSize: 16 }}>{isBn ? 'JPG তৈরি সম্পন্ন!' : 'WebP to JPG Ready!'}</strong>
                      <span style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))', display: 'block' }}>
                        {formatBytes(resultBlob.size)} • Standard Universal JPG
                      </span>
                    </div>
                  </div>
                  <button
      ...
      <section className="tool-content" style={{ marginTop: 40, borderTop: '1px solid hsl(var(--border))', paddingTop: 28 }}>
        <h2>{isBn ? 'কীভাবে WebP ছবিকে JPG তে রূপান্তর করবেন' : 'How to Convert WebP to JPG Online'}</h2>
        <ol>
